...
CREATE SCHEMA IF NOT EXISTS `trocatroca` ;
USE `trocatroca` ;
CREATE TABLE IF NOT EXISTS `trocatroca`.`person` (
...
CREATE TABLE IF NOT EXISTS `trocatroca`.`category` (
...
CREATE TABLE IF NOT EXISTS `trocatroca`.`item` (
  ...
  CONSTRAINT `fk_object_category1`
    ...
    REFERENCES `trocatroca`.`category` (`idcategory`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;

CREATE TABLE IF NOT EXISTS `trocatroca`.`person_adv_donate_item` (
  ...
  CONSTRAINT `fk_person_adv_donate_item_person1`
    ...
    REFERENCES `trocatroca`.`person` (`idperson`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_person_adv_donate_item_item1`
    FOREIGN KEY (`item_iditem` , `item_category_idcategory`)
    REFERENCES `trocatroca`.`item` (`iditem` , `category_idcategory`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;

CREATE TABLE IF NOT EXISTS `trocatroca`.`country` (
  `idcountry` INT NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(60) CHARACTER SET utf8 NULL,
  PRIMARY KEY (`idcountry`))
ENGINE = InnoDB;

CREATE TABLE IF NOT EXISTS `trocatroca`.`FU` (
  ...
  CONSTRAINT `fk_FU_country1`
    ...
    REFERENCES `trocatroca`.`country` (`idcountry`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;

CREATE TABLE IF NOT EXISTS `trocatroca`.`city` (
  ...
  CONSTRAINT `fk_city_FU2`
    ...
    REFERENCES `trocatroca`.`FU` (`idFU` , `country_idcountry`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;

CREATE TABLE IF NOT EXISTS `trocatroca`.`street` (
  ...
  CONSTRAINT `fk_street_city2`
    ...
    REFERENCES `trocatroca`.`city` (`idcity` , `FU_idFU` , `FU_country_idcountry`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;

CREATE TABLE IF NOT EXISTS `trocatroca`.`address` (
  ...
  CONSTRAINT `fk_address_street1`
    ...
    REFERENCES `trocatroca`.`street` (`idstreet` , `city_idcity` , `city_FU_idFU` , `city_FU_country_idcountry`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_address_person1`
    ...
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
CREATE TABLE IF NOT EXISTS `trocatroca`.`person_adv_exch_item` (
